...
def normalization_test(lm, vocab_index):
    ...
    sums to one. 
    Also checks that our methods are implemented correctly: 
        - get_log_prob_sequence (sum of the log probs for each of the next_chars, given context, the next char's context will include all previous next_chars (appended to given context))
        - get_next_char_log_probs (produce probability distribution (list) across all possible chars in our vocab, given context)
    :param lm:
    :param voc:
    :return:
    """
    ...
    normalizes = True
    for context in contexts:
        total_prob_single = np.sum(np.exp(lm.get_next_char_log_probs(context))) # exponentiating log probs gives you the probs
